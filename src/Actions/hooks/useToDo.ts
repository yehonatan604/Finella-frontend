...
const useToDo = (isTodoPage: boolean = false) => {
    ...
    const rows = useMemo(() => {
        return fetchedToDos.map((todo) => ({
            ...
            startDate: todo.startDate.split("T")[0].split("-").reverse().join("-"),
            endDate: todo.endDate.split("T")[0].split("-").reverse().join("-"),
    ...
    const onUpdate = async (data: TToDo) => {
        try {
            ...
            setFetchedTodos((prev) => prev.map((todo) => (todo._id === data._id ? data : todo)));
